...
\tindex{bounded probabilistic polynomial}{오류율을 잡아둘 수 있는
  확률형 다항}
\tindex{bottom}{바닥}
\tindex{built-in}{붙박이}
...
\tindex{determinisitc}{결정적}
\tindex{digit}{숫자}
\tindex{disjoint}{겹침 없는}
...
\tindex{network}{네트웍}
\tindex{node}{노드}
\tindex{non-deterministic}{비결정적}
\tindex{non-expansive}{메모리 반응을 일으키지 않는}
\tindex{normal form}{정규 형태}
\tindex{object}{객체}
\tindex{object}{물건}
\tindex{object-oriented language}{객체 지향 언어}
