...
\tindex{abstract syntax}{문장 조립규칙}
\tindex{abstract syntax}{문장 조립방법}
\tindex{abstract syntax}{문장 조립법}
\tindex{abstract syntax}{문장 구성규칙}
\tindex{abstract syntax}{문장 구성방법}
\tindex{abstract syntax}{문장 구성법}
\tindex{concrete syntax}{구체적 문법구조}
\tindex{concrete syntax}{문장구조 복원규칙}
\tindex{concrete syntax}{문장구조 복원방법}
\tindex{concrete syntax}{문장구조 복원법}
\tindex{concrete syntax}{문장구조 파악규칙}
\tindex{concrete syntax}{문장구조 파악방법}
\tindex{concrete syntax}{문장구조 파악법}
\tindex{abstract type}{구현된 속사정이 감추어진 타입}
\tindex{abstract type}{속내용이 감추어진 타입}
\tindex{abstract type}{핵심만 드러낸 타입}
...
\tindex{applicative language}{값중심 언어}
\tindex{applicative language}{계산중심 언어}
\tindex{applicative language}{함수형 언어}
\tindex{assignment}{메모리에 쓰기}
\tindex{assignment}{값 기록하기}
\tindex{assignment}{기록하기}
\tindex{associativity}{방향성}
\tindex{associativity}{결합법칙}
\tindex{associativity}{결합순서}
\tindex{attribute grammar}{할일이 딸려있는 문법}
\tindex{attribute grammar}{할일딸린 문법}
\tindex{attribute grammar}{일거리가 딸려있는 문법}
\tindex{attribute grammar}{일거리딸린 문법}
\tindex{axiomatic theory}{엄밀한 이론}
\tindex{axiomatic theory}{공리에 기초한 이론}
...
\tindex{bounded quantification}{울타리안에서 일반화시키기}
\tindex{bottom}{바닥}
\tindex{built-in}{붙박이}
\tindex{built-in}{이미 있는}
\tindex{bottom-up program synthesis}{작은 식 조립해서 큰 프로그램 합성하기}
\tindex{bottom-up program synthesis}{부품에서 전체로 프로그램 합성하기}
\tindex{bottom-up program synthesis}{바닥부터 프로그램 합성해가기}
...
\tindex{continuation passing style}{앞으로 할 일 전달하는}
\tindex{continuation passing style}{앞으로 할 일을 함수로 표현해서 전달하는}
...
%\tindex{dangling pointer}{메모리 대상이 사라진 포인터}
...
\tindex{data structure}{자료 정리한 모양}
...
\tindex{delimiter}{구분문자}
\tindex{delimiter}{구별문자}
\tindex{denotational semanitcs}{고정점방식 의미구조}
...
\tindex{dynamic semantics}{프로그램 실행}
\tindex{dynamic semantics}{프로그램 실행 의미구조}
...
%F
\tindex{first-class function}{함수가 특이하게 취급되지않는}
\tindex{first-class function}{특이하게 취급되지않는 함수}
\tindex{first-class function}{차별받지않는 함수}
\tindex{first-class function}{다되는 함수}
...
\tindex{function abstraction}{함수로 핵심 간추리기}
\tindex{function application}{함수 사용}
\tindex{function application}{함수 호출}
\tindex{function argument}{함수 인자}
...
\tindex{functional programming}{값중심 프로그래밍}
\tindex{functional style}{값중심 스타일}
\tindex{functional style}{함수중심 스타일}
...
\tindex{homomorphic encryption}{암호문안에서 연산가능한 암호체계}

%I
\tindex{identifier}{이름}
\tindex{identity function}{일없는 함수}
\tindex{imperative language}{기계중심 언어}
...
\tindex{incomplete}{빠진게 있는}
\tindex{incompleteness theorem}{불완전성 정리}
\tindex{incompleteness theorem}{기계적인 방식으로는 늘 빠뜨린게 있다는 정리}
\tindex{induction}{인덕}
...
\tindex{memoization}{메모해두기}
\tindex{memoization}{기억해두기}
...
\tindex{non-expansive}{새 메모리를 쓰지않는}
\tindex{normal form}{바른꼴}
\tindex{normal form}{표준형}
\tindex{null dereference}{없는주소 접근}
\tindex{null dereference}{헛주소 접근}
\tindex{null}{없는 주소}
\tindex{null}{헛 주소}
...
\tindex{probably approximately correct, PAC}{얼추거의맞는}
\tindex{parameter}{인자}
...
\tindex{predicate abstraction}{논리식을 변수 하나로 요약하기}
\tindex{predicate abstraction}{조건식을 변수 하나로 요약하기}
\tindex{predicate abstraction}{논리조건을 변수 하나로 요약하기}
...
\tindex{sequence}{나열}
\tindex{sequence}{일렬}
\tindex{short-circuit evaluation}{단축계산}
\tindex{short-circuit evaluation}{가로질러 계산하기}
\tindex{side-effect}{부수효과}
\tindex{side-effect}{메모리 반응}
\tindex{side-effect}{수반되는 효과}
\tindex{side-effect}{함께오는 효과}
...
\tindex{static semantics}{프로그램 기획}
...
%T
\tindex{tail recursive}{끝재귀}
\tindex{tail recursive}{꼬리재귀}
\tindex{tail recursive}{마지막에 자기자신을 부르는}
\tindex{tail recursive}{자기 호출이 마지막인}
\tindex{template}{거푸집}
\tindex{term}{식}
\tindex{top declaration}{가장 위 선언}
\tindex{top-level declaration}{가장 위 선언}
...
\tindex{effect type}{값종류 이외의 성질을 추적하는 타입}
...
\tindex{top-down program synthesis}{위에서부터 프로그램 합성해하기}
\tindex{top-down program synthesis}{전체에서 부품으로 프로그램 합성하기}
...
\tindex{univerally quantified type}{타입변수가 모든 타입으로 바꿔치기될 수 있는}
\tindex{existentially quantified type}{타입변수가 어떤 타입으로 바꿔치기될 수 있는}
...
\tindex{variant type}{모둠 타입}
\tindex{variant type}{갈래 타입}
